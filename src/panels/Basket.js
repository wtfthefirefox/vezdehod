...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  ...
  useEffect( () => {
    let basketSettings;
    if (JSON.parse(localStorage.getItem('basketSettings'))) {
      let valueBasketSettings = JSON.parse(localStorage.getItem('basketSettings'));
    ...
    setFaster(basketSettings.timeMode);
    setTime(basketSettings.timeValue);
    setSelfService(basketSettings.selfService);

    localStorage.setItem('basketSettings', JSON.stringify(basketSettings));
  }, []);
    ...
    <div className="Place">
      ...
      <div className="Place__products-wrapper">
        ...
        <Link
          ...
          onClick={() => {
            const localBasketSettings = {
              timeValue: time,
              timeMode: faster,
              selfService
            };
            localStorage.setItem('basketSettings', JSON.stringify({...localBasketSettings, }));
          }}
        >
          Изменить
        </Link>
