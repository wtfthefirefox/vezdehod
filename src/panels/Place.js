  ...
  return (
    <div className="Place">
      ...
      <footer className="Place__footer">
        {
          price !== '0' ?  <Link to={`/basket/${area.id}/${item.id}`} className="Place__order">Оформить заказ ({price})</Link>: <div className="Place__order">Для оформления заказа вам надо выбрать хотя бы один товар</div>
        } 
      </footer>
    </div>
  );
